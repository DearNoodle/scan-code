        ...
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-2.5 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              disabled={busy}
              className="rounded-lg bg-zinc-900 px-3.5 py-2 text-sm font-medium text-white shadow-sm hover:bg-zinc-800 disabled:opacity-50"
            ...
            <button
              type="button"
              className="rounded-lg border border-zinc-300 bg-white px-3.5 py-2 text-sm text-zinc-700 hover:bg-zinc-50"
            ...
            >
              <div className="flex w-40 shrink-0 flex-col gap-1.5 sm:w-44">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {row.error ? (
                    <span className="truncate text-xs text-red-600">Invalid</span>
                  ) : row.stale ? (
                    <span className="truncate text-xs text-amber-700">Stale</span>
                  ) : row.image ? (
                    <span className="truncate text-xs text-emerald-700">Ready</span>
                  ) : (
                    <span className="truncate text-xs text-zinc-400">Empty</span>
                  )}
                </div>
              </div>
              ...
              <div className="min-w-0 flex-1">
                <div
                  className={`relative flex h-full min-h-[7rem] w-full items-center justify-center overflow-hidden rounded-lg border bg-white ${
                    row.error
                ...
                >
                  <button
                    type="button"
                    className="absolute bottom-1.5 right-1.5 z-10 flex h-7 w-7 items-center justify-center rounded-md border border-red-200 bg-white/90 text-red-600 shadow-sm hover:bg-red-50 disabled:opacity-30"
                  ...
                  {row.image && !row.error ? (
                    <button
                      ...
                      className="absolute top-1.5 right-1.5 z-10 flex h-7 w-7 items-center justify-center rounded-md border border-zinc-200 bg-white/90 text-zinc-600 shadow-sm hover:bg-white"
                  ...
                  {row.error ? (
                    ...
                    <img
                      src={row.image}
                      alt=""
                      className={`max-h-full max-w-full object-contain ${row.stale ? 'opacity-45' : ''}`}
                    />
                  ) : (
